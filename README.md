...
- [関数](#関数)
    - [単一式関数](#単一式関数)
    - [デフォルト引数・名前付き引数](#デフォルト引数名前付き引数)
    - [可変長引数](#可変長引数)
    - [Unit（戻り値なし）](#unit戻り値なし)
    - [再帰・末尾再帰](#再帰末尾再帰)
    - [ローカル関数](#ローカル関数)
- [ラムダと高階関数](#ラムダと高階関数)
    - [ラムダ式](#ラムダ式)
    - [it — 単一パラメータの省略](#it--単一パラメータの省略)
    - [高階関数](#高階関数)
    - [末尾ラムダ（Trailing Lambda）](#末尾ラムダtrailing-lambda)
    - [関数参照](#関数参照)
    - [クロージャ（外部変数のキャプチャ）](#クロージャ外部変数のキャプチャ)
- [クラス](#クラス)
    - [init ブロック・セカンダリコンストラクタ](#init-ブロックセカンダリコンストラクタ)
    - [プロパティ](#プロパティ)
    - [アクセス修飾子](#アクセス修飾子)
- [継承](#継承)
...
- [データクラス](#データクラス)
- [オブジェクト](#オブジェクト)
    - [オブジェクト宣言（シングルトン）](#オブジェクト宣言シングルトン)
    - [コンパニオンオブジェクト](#コンパニオンオブジェクト)
    - [匿名オブジェクト](#匿名オブジェクト)
- [インターフェース](#インターフェース)
- [列挙型](#列挙型)
    - [プロパティ付き enum](#プロパティ付き-enum)
    - [enum のメソッド](#enum-のメソッド)
- [シールドクラス](#シールドクラス)
- [Null 安全](#null-安全)
    - [Nullable 型](#nullable-型)
    - [セーフコール演算子 ?.](#セーフコール演算子-)
    - [エルビス演算子 ?:](#エルビス演算子-)
    - [Not-null アサーション !!](#not-null-アサーション-)
    - [スマートキャスト](#スマートキャスト)
    - [セーフキャスト as?](#セーフキャスト-as)
    - [let との組み合わせ](#let-との組み合わせ)
- [制御フロー](#制御フロー)
    - [if 式](#if-式)
    - [when 式](#when-式)
    - [for ループ](#for-ループ)
    - [while ループ](#while-ループ)
    - [break / continue / return ラベル](#break--continue--return-ラベル)
- [拡張関数・拡張プロパティ](#拡張関数拡張プロパティ)
    - [拡張関数](#拡張関数)
    - [ジェネリック拡張関数](#ジェネリック拡張関数)
    - [拡張プロパティ](#拡張プロパティ)
- [スコープ関数](#スコープ関数)
...
- [ジェネリクス](#ジェネリクス)
    - [型制約](#型制約)
    - [分散](#分散)
    - [具体化型パラメータ（reified）](#具体化型パラメータreified)
- [エラー処理](#エラー処理)
    - [例外のスロー](#例外のスロー)
    - [try 式](#try-式)
    - [runCatching](#runcatching)
- [並行処理（コルーチン）](#並行処理コルーチン)
    - [基本的なコルーチン起動](#基本的なコルーチン起動)
    - [async / await](#async--await)
    - [suspend 関数](#suspend-関数)
    - [Flow（非同期ストリーム）](#flow非同期ストリーム)
    - [コルーチンコンテキスト・ディスパッチャー](#コルーチンコンテキストディスパッチャー)
- [Java 相互運用性](#java-相互運用性)
    - [@JvmStatic / @JvmField](#jvmstatic--jvmfield)
...
| String | 文字列 |
| UByte | 符号なし 8bit 整数型 |
| UShort | 符号なし 16bit 整数型 |
| UInt | 符号なし 32bit 整数型 |
| ULong | 符号なし 64bit 整数型 |
| Any | すべての型のスーパータイプ |
| Unit | Java の void に相当 |
...
```kotlin
val language = "Kotlin"   // String 型と推論
var count = 0             // Int 型と推論
```

### 遅延初期化

```kotlin
lateinit var name: String      // var のみ、null 非許容型に使用
...
listOf(1, 2, 3).map { it * 2 }   // it は暗黙的なパラメータ名
```
...
```kotlin
var a: String = "hello"   // null 不可
var b: String? = null     // null 許容
```

### セーフコール演算子 ?.

```kotlin
val length = b?.length            // b が null なら null を返す
...
```kotlin
val len = b!!.length   // null なら NullPointerException
```
...
```kotlin
if (b != null) {
    println(b.length)   // null 非許容として扱われる
}
```
...
```kotlin
val str: String? = obj as? String   // キャスト失敗時は null
```
...
| ディスパッチャー | 説明 |
|---|---|
| Dispatchers.Default | CPU 集中タスク |
| Dispatchers.IO | I/O タスク（ファイル、ネットワーク） |
| Dispatchers.Main | UI スレッド（Android など） |
...
```kotlin
// Java クラスを Kotlin から使用
import java.util.ArrayList

val list = ArrayList<String>()
list.add("Kotlin")

// Java の static メソッドへのアクセス
...
// Java から: MyClass.create(), MyClass.TAG
```
...
// Java からオーバーロードとして見える
```

## 参考資料

- [Kotlin 公式ドキュメント（英語）](https://kotlinlang.org/docs/home.html)
- [Android 向け Kotlin 入門（日本語）](https://developer.android.com/kotlin/learn?hl=ja)
- [Kotlin 言語仕様](https://kotlinlang.org/spec/)
- [Kotlin 標準ライブラリ API](https://kotlinlang.org/api/latest/jvm/stdlib/)
